...
export default class InfoPanelShipCpt extends React.Component {
  ...
  initializeShipData = () => {
    ...
    this.shipData.pilotSkills.sort(
      (firstEle, secondEle) => firstEle - secondEle,
    );
  };
  ...
  getPilotSkillsString = () => {
    ...
    pilotSkillString +=
      this.shipData.pilotSkills[this.shipData.pilotSkills.length - 1];
  ...
  render() {
    ...
    return (
      <div>
        ...
        <div>
          <strong>Base: </strong>
          <span>{xwingUtils.getShipBaseSize(this.shipData)}</span>
        </div>
        <div>
          <strong>Initiative: </strong>
          <span className="info-initiative">{this.getPilotSkillsString()}</span>
        </div>
        <div>
          <strong>Points: </strong>
          <span>{`${this.shipData.minPilotCost} - ${this.shipData.maxPilotCost}`}</span>
        </div>
        <div>
