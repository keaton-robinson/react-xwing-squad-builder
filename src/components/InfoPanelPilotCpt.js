...
export default class InfoPanelPilotCpt extends React.Component {
  ...
  render() {
    ...
    if (this.props.isSelectedPilot) {
      ...
      pilotBaseStats = xwingUtils.getAppReadyPilot(
        this.props.pilot,
        xwingData.ships,
      );
    }

    const pilotEffectiveStats = xwingUtils.getPilotEffectiveStats(
      pilotBaseStats,
      xwingData.upgrades,
    );
    ...
    return (
      <div>
        ...
        <div>
          <strong>Base: </strong>
          <span>
            {xwingUtils.getShipBaseSize(pilotEffectiveStats.pilotShip)}
          </span>
        </div>
        <div>
        ...
        <div
          ...
          dangerouslySetInnerHTML={{
            __html: this.getRulesMarkupForPilot(pilotEffectiveStats),
          }}
        />
        <div>
          <ManeuversCpt
