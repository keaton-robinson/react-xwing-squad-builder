            ...
            {squad.map(selectedPilot => 
                <table key={selectedPilot.uiKey} style={{marginBottom: '30px', minWidth: '500px'}}>
                    <tbody>
                        ...
                        <tr></tr>
                        <tr>
                            <td><strong>{`Half Points: ${Math.ceil(getTotalCost(selectedPilot)/2)}   Threshold: ${Math.ceil((xwingUtils.getPilotEffectiveStats(selectedPilot, xwingData.upgrades).pilotShip.hull + xwingUtils.getPilotEffectiveStats(selectedPilot, xwingData.upgrades).pilotShip.shields)/2)}`}</strong></td>
                            <td style={{textAlign:'right'}}><strong>{`Ship Total: ${getTotalCost(selectedPilot)}`}</strong></td>
                        </tr>
                    </tbody>
                </table>
            )}
            <div style={{textAlign:'right'}}><button className='btn-primary no-print' onClick={print}><i className="fa-solid fa-print"></i> Print</button></div>
        </div>
    );
}
